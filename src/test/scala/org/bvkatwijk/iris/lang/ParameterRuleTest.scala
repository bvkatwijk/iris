...
class ParameterRuleTest extends ParseTest {
  "Parameter" - {
    "accepts" - {
      def accepts(value: String, result: Parameter) = run(value) should be(Right(result))
      "a: B" in accepts("a: B", Parameter("a", QualifiedIdentifier(None, Identifier("B"))))
      "name: Type" in accepts("name: Type", Parameter("name", QualifiedIdentifier(None, Identifier("Type"))))
      "no:Space" in accepts("no:Space", Parameter("no", QualifiedIdentifier(None, Identifier("Space"))))
    }
    "denies" - {
